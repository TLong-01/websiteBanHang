...
//				sql += " and p.status=1 ";
	...
	public PagerData<Product> search1(OrderSearchModel searchModel) {
		String sql = "SELECT * FROM tbl_products p WHERE 1=1";
		if (searchModel != null) {
			if (searchModel.orderId != null) {
				sql += " and p.category_id = " + searchModel.orderId;
			}
...
//				sql += " and (p.saleorder_id like '%" + searchModel.keyword + "%')";
	...
	public PagerData<Product> getById(OrderSearchModel searchModel) {
		String sql = "SELECT * FROM tbl_products p WHERE ";
		if (searchModel != null) {
			if (searchModel.orderId != null) {
				sql += " p.category_id = " + searchModel.orderId;
			}
		}
